...
import { requestOtp, login } from '@/services/auth.service';
...
const LoginPage: React.FC = () => {
  ...
  const [otpStep, setOtpStep] = useState<'request' | 'submit'>('request');
  ...
  const renderPasswordForm = () => (
    <form onSubmit={handleLogin} className="space-y-6">
      ...
      <div className="text-center">
        <button type="button" onClick={() => setAuthMethod('otp')} className="text-sm text-blue-600 hover:underline">
          Login with Code
        </button>
      </div>
    </form>
    ...
    <div>
      {otpStep === 'request' ? (
        <div className="space-y-6">
          ...
          />
          {error && <p className="text-red-500 text-xs italic">{error}</p>}
          <SGCButton onClick={handleRequestOtp} disabled={loading} className="w-full">
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <SGCOtpInput length={6} value={otp} onChange={setOtp} disabled={loading} />
          </div>
          {error && <p className="text-red-500 text-xs italic">{error}</p>}
          <SGCButton type="submit" disabled={loading} className="w-full">
      ...
      <div className="text-center mt-4">
        <button type="button" onClick={() => setAuthMethod('password')} className="text-sm text-blue-600 hover:underline">
          Login with Password
        </button>
      </div>
    </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
          ...
          <AnimateGSAP className="flex justify-center">
            <SGCCard title="Login" className="w-full max-w-md">
              {authMethod === 'password' ? renderPasswordForm() : renderOtpForm()}
              <div className="text-xs text-slate-400 mt-4">By continuing you agree to our <a className="underline" href="/terms">Terms</a> and <a className="underline" href="/privacy">Privacy Policy</a>.</div>
