          ...
          <table className="min-w-full bg-white text-gray-800">
            <thead>
              <tr className="bg-gray-50">
                ...
                <th className="py-2 px-4 text-left font-semibold">Symbol</th>
            ...
            <tbody>
              {tokens.length === 0 ? (
                <tr>
                  <td colSpan={5} className="text-center py-4">You haven't created any tokens yet.</td>
                </tr>
              ) : (
                tokens.map(token => (
                  <tr key={token._id} className="border-b border-gray-200">
                    <td className="py-2 px-4">{token.metadata.name}</td>
                    <td className="py-2 px-4">{token.metadata.symbol}</td>
                    <td className="py-2 px-4">{token.tier}</td>
