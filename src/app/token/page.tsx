      ...
      <div className="mb-6">
        <AnimateGSAP>
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold">Create & Manage Tokens</h1>
              <p className="text-sm text-slate-600">Launch tokens, manage drafts, and view status.</p>
            </div>

            {/* <div>
              <SGCButton onClick={() => setIsCreating(s => !s)} className="hidden md:inline-flex">{isCreating ? 'Close' : 'Create Token'}</SGCButton>
            </div> */}
          </div>
        </AnimateGSAP>
      </div>
        ...
        <AnimateGSAP>
          {isCreating ? (
            ...
            <SGCCard className="sgc-glass rounded-xl p-6">
              ...
              <SGCButton onClick={() => setIsCreating(true)}>Start Creating</SGCButton>
            </SGCCard>
          )}
        </AnimateGSAP>
      </div>
