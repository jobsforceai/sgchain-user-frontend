...
import { createChart, IChartApi, ISeriesApi, AreaData, Time, CrosshairMode } from 'lightweight-charts';
...
const SGCPriceChart: React.FC<Props> = ({ height = 300 }) => {
  ...
  const visibleRangeRef = React.useRef<any | null>(null);
  ...
  React.useEffect(() => {
    ...
    if (candles.length > 0 && chartContainerRef.current && !chartRef.current) {
      ...
      const chart = createChart(chartContainerRef.current, {
        ...
        timeScale: { borderColor: '#444', timeVisible: true, secondsVisible: true, lockVisibleTimeRangeOnResize: true },
        watermark: { visible: false },
        crosshair: { mode: CrosshairMode.Normal },
      });
      // Ensure the time scale keeps the current visible range when the chart container is resized
      try {
        chart.timeScale().applyOptions({ lockVisibleTimeRangeOnResize: true });
      } catch (e) {
        // ignore if timeScale not available for some reason
      }
  ...
  // subscribe to visible range changes so we can restore after resize/setData
  const visibleHandler = (visibleRange: any) => {
  ...
  return () => {
    ...
    try {
      chart.timeScale().unsubscribeVisibleTimeRangeChange(visibleHandler as any);
    } catch (e) {
      // ignore
    }
    if (chartRef.current) {
  ...
  // If a visible range has been captured, ensure resize doesn't discard it
  React.useEffect(() => {
    if (!chartRef.current) return;
    const chart = chartRef.current;
    const handleResizeRestore = () => {
      if (!chart || !visibleRangeRef.current) return;
      try {
    ...
    window.addEventListener('resize', handleResizeRestore);
    return () => window.removeEventListener('resize', handleResizeRestore);
  }, []);
